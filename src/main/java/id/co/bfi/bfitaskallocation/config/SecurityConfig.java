  ...
  protected void configure(HttpSecurity http) throws Exception {
    http.csrf().disable()
        ...
        .addFilterAfter(
            ...
            new AuthenticationFilter(authenticationService), UsernamePasswordAuthenticationFilter.class)

        // Authorization requests config
        .authorizeRequests()

        // Enabled swagger end points
        .antMatchers("/api-docs", "/api-docs/**", "/configuration/**", "/swagger*/**", "/webjars/**").permitAll()

        // Allow OPTIONS call for preflight requests
        .antMatchers(HttpMethod.OPTIONS).permitAll()

        // Allow internal endpoints for service users only !
        .antMatchers("/**").hasAuthority(AuthenticationClaim.SYSTEM_SERVICE.toString()).anyRequest().authenticated();
  }
}
